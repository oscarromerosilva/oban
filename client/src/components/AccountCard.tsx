...
export default function AccountCard({ account, variant = 'default' }: AccountCardProps) {
  ...
  let containerClasses =
    'p-6 rounded-2xl relative overflow-hidden transition-all duration-300 group ';
  ...
  if (isPrimary) {
    ...
    containerClasses +=
      'bg-card border border-border shadow-lg shadow-black/5 hover:shadow-xl hover:border-primary/30 h-full';
  }
    ...
    <div className={containerClasses}>
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        <div className="flex justify-between items-start">
          <div>
            <p
              className={`text-sm font-medium ${isPrimary || isSecondary ? 'text-white/70' : 'text-muted-foreground'}`}
            >
              {account.accountType}
            </p>
            <p
              className={`font-mono text-sm mt-1 ${isPrimary || isSecondary ? 'text-white/90' : 'text-foreground/80'}`}
            >
              {maskedNumber}
            </p>
          </div>
        ...
        <div className="mt-8">
          <p
            className={`text-sm mb-1 ${isPrimary || isSecondary ? 'text-white/70' : 'text-muted-foreground'}`}
          >
            Available Balance
          </p>
          <h3
            className={`font-display font-bold text-3xl tracking-tight ${isPrimary || isSecondary ? 'text-white' : 'text-foreground'}`}
          >
            {formattedBalance}
          </h3>
        </div>
